...
class AddMedicalOrganizationSchema(BaseModel):
    name: str = Field(..., max_length=256, description="Name on default language")
    organization_code: str = Field(
        ..., max_length=20, description="Medical organization internal code"
    )
    ...
    address_locales: Dict[str, str] = Field(
        ...
        description="A dictionary of the additional address locales",
    )
    ...
    def _check_address_not_blank(cls, v: Any) -> str:
        validate_field_not_blank(v, "address")
        return v

    _check_lang_and_locales = model_validator(mode="before")(validate_lang_and_locales)
    _validate_addresses_and_locales = model_validator(mode="before")(
        validate_addresses_and_locales
    )


class UpdateMedicalOrganizationSchema(BaseModel):
    name: Optional[str] = Field(
        None, max_length=256, description="Name on default language"
    )
    organization_code: Optional[str] = Field(
        None, max_length=20, description="Medical organization internal code"
    )
    address: Optional[str] = Field(
        None, max_length=256, description="Address on default language"
    )
