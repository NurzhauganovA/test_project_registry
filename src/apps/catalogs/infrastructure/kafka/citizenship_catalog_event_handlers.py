...
citizenship_catalog_eventer_listener = EventerListener(
    ...
    where={
        ...
        "payload": {"model": "cat_citizenship"},
    },
)


@citizenship_catalog_eventer_listener.get(where={"action": EventerActionEnum.CREATE})
@inject
@handle_kafka_event("create", "cat_citizenship")
async def _handle_create_event(
    _raw_message: bytes,
    schema_data: EventerResponseSchema,
    service: CitizenshipCatalogService = Provide[CatalogsContainer.citizenship_catalog_service],
    logger: LoggerService = Provide[CatalogsContainer.logger],
):
    create_schema = map_event_payload_to_schema(schema_data, AddCitizenshipSchema)
    await service.add_citizenship(create_schema)


@citizenship_catalog_eventer_listener.get(where={"action": EventerActionEnum.UPDATE})
@inject
@handle_kafka_event("update", "cat_citizenship")
async def _handle_update_event(
    ...
    update_schema = map_event_payload_to_schema(schema_data, UpdateCitizenshipSchema)
    await service.update_citizenship(update_schema.id, update_schema)


@citizenship_catalog_eventer_listener.get(where={"action": EventerActionEnum.DELETE})
@inject
...
async def _handle_delete_event(
    ...
    delete_schema = map_event_payload_to_schema(schema_data, DeleteCitizenshipSchema)
